...
tfoot td{font-weight:700;background:rgba(0,0,0,0.1);color:var(--gold)}
...
.auth,.app{max-width:1100px;margin:auto}
.hidden{display:none}
...
<div class="auth">
  <h1>💰 Tabungan Emas Pegadaian — Login/Register</h1>
...
<button id="logoutBtn" class="btn ghost small" style="float:right">Logout</button>
...
<div style="text-align:center;margin-top:12px;color:var(--muted)">
  Data tersimpan otomatis di browser (<b>goldAssets_user</b>)
</div>
</div>
...
function loadUserAssets() {
  ...
  if (!$("currentPrice").value) {
    $("currentPrice").value = assets.length ? assets[0].priceBuy : 999000;
  }
  render();
  renderLeftTotals();
  $("currentPrice").dispatchEvent(new Event('input'));
}
...
// ================== EMAS FUNCTIONS ==================
function save(){ if(LS_KEY) localStorage.setItem(LS_KEY,JSON.stringify(assets)); }
...
function render(){
  ...
  if(filterMode==="old") filtered = filtered.filter(a=>a.type==="admin" || (isOld(a.date) && (a.grams- assets.filter(x=>x.type==="sell" && x.parentId===a.id).reduce((s,x)=>s+x.grams,0))>0));
  filtered.sort((a,b)=> new Date(a.date)-new Date(b.date));
  filtered.forEach((a,i)=>{
    if(a.type==="admin"){
      const trAdmin=document.createElement("tr"); trAdmin.dataset.id=a.id; trAdmin.style.color="var(--red)";
    ...
    const soldGram = sold.reduce((s,x)=>s+x.grams,0); const rem = a.grams-soldGram; const nowVal = rem*(currentPrice||a.priceBuy); const profit=nowVal-rem*a.priceBuy;
...
tbody.addEventListener("click", e => {
  ...
  if(e.target.classList.contains("editBtn") && a.type==="buy"){
    const newGram= +prompt("Ubah gram:", a.grams); const newPrice= +prompt("Ubah harga beli/gr:", a.priceBuy); const newDate= prompt("Ubah tanggal (YYYY-MM-DD):", a.date);
  ...
  if(e.target.classList.contains("editAdmin") && a.type==="admin"){
    const g=+prompt("Ubah gram admin (angka minus):", a.grams); const d=prompt("Tanggal admin (YYYY-MM-DD):", a.date);
...
$("restoreFile").onchange=function(e){ const f=e.target.files[0]; if(!f)return; const r=new FileReader(); r.onload=ev=>{ try{ const parsed=JSON.parse(ev.target.result); if(Array.isArray(parsed)){ assets=parsed.map(normalizeItem); save(); render(); renderLeftTotals(); alert("✅ Data berhasil dipulihkan."); } else alert("File JSON tidak berisi array transaksi."); }catch(err){ console.error(err); alert("Gagal membaca JSON."); } }; r.readAsText(f); };
